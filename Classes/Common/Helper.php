...
class Helper
{
    ...
    public static function processDatabaseAsAdmin(array $data = [], array $cmd = [], bool $reverseOrder = false, bool $cmdFirst = false): array
    {
        ...
        // phpcs:ignore Squiz.PHP.Superglobals.SuperGlobalUsage,WordPress.WP.GlobalVariablesOverride.Prohibited -- TYPO3 stores the current PSR-7 request in the global runtime state.
        $request = $GLOBALS['TYPO3_REQUEST'] ?? null;
        $isBackendRequest = $request !== null && ApplicationType::fromRequest($request)->isBackend();

        if ($isCli) { // Initialize CLI user if not already done (e.g. by a CommandController)
            // phpcs:ignore Squiz.PHP.Superglobals.SuperGlobalUsage,WordPress.WP.GlobalVariablesOverride.Prohibited -- TYPO3 exposes the backend user through global runtime state.
            ...
            if (empty($backendUser->user['uid'] ?? null) || !$backendUser->isAdmin()) {
        ...
        if (($isCli && isset($backendUser) && $backendUser->isAdmin()) || ($isBackendRequest && $context->getPropertyFromAspect('backend.user', 'isAdmin'))) {
